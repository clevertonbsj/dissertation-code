...
def order_queue_edd(task_q, elapsed_time, Task_dictionary, capacity, n_agents):
    ...
    for j in range(n_agents):
        task_q_ord.extend(total_task_q_ord[j::n_agents])
    return task_q_ord
...
def Do(adjacency_matrix, non_split_tasks, tasks_dictionary, elapsed_time,
    ...
    for agent in range(n_agents):
        ...
        while i < len(task_queue): #keeps the function running while going through task queue
            ...
            if Load == True:
                ...
                #block for adding the movement restriction
    ...
    return non_split_tasks, idle_task, elapsed_time, curr_pos, complete, failed, time_taken, start_times
...
class selector_brain(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        super(selector_brain, self).__init__()
...
class master_brain(nn.Module):
    ...
    def __init__(self, input_size, output_size, z):
        ...
        if z == 0:
            ...
            self.fc7 = nn.Linear(int(self.input_size*1), int(self.input_size*.8))
            self.fc8 = nn.Linear(int(self.input_size*.8), int(self.input_size*.6))
            self.fc9 = nn.Linear(int(self.input_size*.6), int(self.output_size*.4))
            self.fc10 = nn.Linear(int(self.output_size*.4), int(self.output_size))
            #self.fc11 = nn.Linear(int(self.output_size*1.2), int(self.output_size*1.4))
            #self.fc12 = nn.Linear(int(self.output_size*1.4), int(self.output_size*1.6))
            #self.fc13 = nn.Linear(int(self.output_size*1.6), int(self.output_size*1.8))
            #self.fc14 = nn.Linear(int(self.output_size*1.8), int(self.output_size*2))
            ...
            self.fc8 = nn.Linear(int(self.input_size*.8), int(self.input_size*.6))
            self.fc9 = nn.Linear(int(self.input_size*.6), int(self.output_size))
            #self.fc10 = nn.Linear(int(self.output_size*.4), int(self.output_size*1.2))
            #self.fc11 = nn.Linear(int(self.output_size*1.2), int(self.output_size*1.4))
            #self.fc12 = nn.Linear(int(self.output_size*1.4), int(self.output_size*1.6))
            #self.fc13 = nn.Linear(int(self.output_size*1.6), int(self.output_size*1.8))
            #self.fc14 = nn.Linear(int(self.output_size*1.8), int(self.output_size*2))
...
class agent_brain(nn.Module):
    ...
    def __init__(self, input_size, output_size, z):
        ...
        if z == 0:
            ...
            self.fc14 = nn.Linear(int(self.output_size*1.8), int(self.output_size*2))
            self.fc15 = nn.Linear(int(self.output_size*2), int(self.output_size*2.2))
            self.fc16 = nn.Linear(int(self.output_size*2.2), int(self.output_size))
            #self.fc17 = nn.Linear(int(self.output_size*2.4), int(self.output_size*2.6))
            #self.fc18 = nn.Linear(int(self.output_size*2.6), int(self.output_size*2.8))
            #self.fc19 = nn.Linear(int(self.output_size*2.8), int(self.output_size*3))
            #self.fc20 = nn.Linear(int(self.output_size*3), int(self.output_size))
            ...
            self.fc15 = nn.Linear(int(self.output_size*2), int(self.output_size*2.2))
            self.fc16 = nn.Linear(int(self.output_size*2.2), int(self.output_size*2.4))
            self.fc17 = nn.Linear(int(self.output_size*2.4), int(self.output_size))
            #self.fc18 = nn.Linear(int(self.output_size*2.6), int(self.output_size*2.8))
            #self.fc19 = nn.Linear(int(self.output_size*2.8), int(self.output_size*3))
            #self.fc20 = nn.Linear(int(self.output_size*3), int(self.output_size))
        elif z == 3:
            ...
            self.fc16 = nn.Linear(int(self.output_size*2.2), int(self.output_size*2.4))
            self.fc17 = nn.Linear(int(self.output_size*2.4), int(self.output_size*2.6))
            self.fc18 = nn.Linear(int(self.output_size*2.6), int(self.output_size))
            #self.fc19 = nn.Linear(int(self.output_size*2.8), int(self.output_size*3))
            #self.fc20 = nn.Linear(int(self.output_size*3), int(self.output_size))
        elif z == 5:
            self.fc1 = nn.Linear(self.input_size, int(self.input_size*2))
            ...
            self.fc17 = nn.Linear(int(self.output_size*2.4), int(self.output_size*2.6))
            self.fc18 = nn.Linear(int(self.output_size*2.6), int(self.output_size*2.8))
            self.fc19 = nn.Linear(int(self.output_size*2.8), int(self.output_size))
            #self.fc20 = nn.Linear(int(self.output_size*3), int(self.output_size))
        else:
            self.fc1 = nn.Linear(self.input_size, int(self.input_size*2))
            self.fc2 = nn.Linear(int(self.input_size*2), int(self.input_size*1.8))
...
class Dqn():
    ...
    def __init__(self, gamma, input_vector, bias, current_position, loss, agent_number, brain, z):
        self.input_vector = []
        self.current_position = current_position
        ...
        self.reward_window = []
        if brain == 'master':
            self.model = master_brain(len(self.input_vector), len(self.bias), z)
            self.loss = loss
            ...
            self.model = agent_brain(len(self.input_vector), len(self.bias), z)
            self.loss = loss
    ...
    def create_bias(self, state, z):
        bias = self.model(state, z)
        return bias
    
    def learn(self, batch_state, batch_next_state, batch_reward, batch_bias, z):
        bias = self.model(batch_state, z).squeeze(1)
        next_bias = self.model(batch_next_state, z).detach()
    ...
    def update(self, reward, last_state, z):
        ...
        self.bias = self.create_bias(new_state, z)
        if len(self.memory.memory) > 100:
            batch_state, batch_next_state, batch_reward, batch_bias = self.memory.sample(100)
            self.learn(batch_state, batch_next_state, batch_reward, batch_bias, z)
        self.last_bias = self.bias
    ...
    if type == 'agent':
        ...
        for i in range(len(total_task_queue)):
            ...
            if task != 'Dummy':
                ...
                # unload_pos = task_dict[task][1] + warehouse_number
                #task_load_move.append(adjacency_matrix[current_position][load_pos])
                task_load_start.append(elapsed_time)
                task_load_end.append(99 + task_load_start[i])
                #task_unload_move.append(adjacency_matrix[unload_pos][load_pos])
                task_unload_start.append(task_load_end[i])
                task_unload_end.append(99 + task_unload_start[i])
...
def Train_model(task_q, task_scores, adjacency_matrix, trainning_duration, Task_dictionary, 
                warehouse_number, gamma, trigger, agent_capacity, n_agents, master_loss, agent_loss,
                previous_duration, z, selector = False):
    scores = []
    total_penalty = 0
    elapsed_time = 0
    ...
    agent = Dqn( gamma, agent_input_vector, agent_bias, current_position, 
                agent_loss, n_agents, 'agent', z)
    if check_brain('master ' + str(previous_duration)+ ' ' + str(z)):
        master.load('master ' + str(previous_duration)+ ' ' + str(z))
    if check_brain('agent ' + str(previous_duration)+ ' ' + str(z)):
        agent.load('agent ' + str(previous_duration)+ ' ' + str(z))
    ...
    while t_f - t_i <= t_d:
        ...
        if not selector:
            ...
            master_bias = master.update(last_reward, master_last_state, z)
        ...
        for i in range(n_agents):
            ...
            agent_last_state = create_input(agent_task_q, Task_dictionary, agent_bias, current_position[i], 
                                elapsed_time, adjacency_matrix, warehouse_number, n_agents, 'agent')
            agent_bias = agent.update(last_reward, agent_last_state, z)
    ...
    master.save('master ' + str(t_d)+ ' ' + str(z))
    agent.save('agent '+ str(t_d)+ ' ' + str(z))
...
def create_queue2(task_queue, Task_dictionary, current_position, elapsed_time,
                ...
                scores, agent_capacity, n_agents, master_loss, agent_loss, training_duration, z, selector = False):
    ...
    master_input_vector = create_input(task_queue, Task_dictionary, master_bias, 0,
                                       ...
                                       n_agents, 'master')
    selector_split = len(master_input_vector)/len(task_queue)
    master = Dqn( gamma, master_input_vector, master_bias, 0, master_loss, agent_loss, 'master', z)
    master.load('master ' + str(training_duration)+ ' ' + str(z))
    if not selector:
            master_last_state = master_input_vector
            master_bias = master.update(last_reward, master_last_state, z)
            master_bias = master_bias.tolist()[0]
    ...
    agent_input_vector = create_input(agent_task_q, Task_dictionary, agent_bias, 0, 
                                elapsed_time, adjacency_matrix, warehouse_number, n_agents, 'agent')
    agent = Dqn( gamma, agent_input_vector, agent_bias, current_position, master_loss, agent_loss, 'agent', z)
    agent.load('agent ' + str(training_duration)+ ' ' + str(z))
    ...
    for i in range(n_agents):            
            ...
            agent_input = create_input(agent_task_q, Task_dictionary, agent_bias, current_position[i], 
                                elapsed_time, adjacency_matrix, warehouse_number, n_agents, 'agent')
            agent_bias = agent.update(last_reward, agent_input, z)
            agent_bias = agent_bias.tolist()[0]
