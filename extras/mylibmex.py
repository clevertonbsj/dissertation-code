...
def join_q_bias(split_queue, split_bias):
    L = []
    L_i = []
    U = []
    U_i = []
    ...
    for task in split_queue:
        ...
        if t_type == 'L':
            if t_id != 'Dummy':
                ...
                L.append([t_id])
            L_i.append(split_queue.index(task))
        else:
            if t_id != 'Dummy':
                U.append([int(t_id)])
            else:
                U.append([t_id])
            U_i.append(split_queue.index(task))
    for i in range(len(L)):
        L_bias = split_bias[L_i[i]]
        u_i = U.index(L[i])
        U_bias = split_bias[U_i[u_i]]
        task_bias = np.mean([L_bias, U_bias])
        final_bias.append(task_bias)
        final_queue.append(L[i])
        U[u_i] = None
    return final_queue, final_bias
...
def Train_model(task_q, task_scores, adjacency_matrix, trainning_duration, Task_dictionary, 
    ...
    if len(task_queue) < agent_capacity * n_agents:
        for i in range((agent_capacity * n_agents) - len(task_q)):
            task_queue.append(['Dummy'])
    ...
    if not selector:
        ...
        master_input_vector = create_input(task_queue, Task_dictionary, master_bias, 0, 
                                    elapsed_time, adjacency_matrix, warehouse_number,
                                    n_agents, 'master')
        selector_split = int(len(master_input_vector)/len(task_queue))
    ...
    while t_f - t_i <= t_d:
        ...
        final_queue = [queue for sublist in aux_queue for queue in sublist]
        final_bias = [bias for sublist in aux_bias for bias in sublist]
        master_task_q, final_bias = join_q_bias(final_queue, final_bias)
        for i in range(n_agents):
        ...
        master_input_vector = create_input(master_task_q, Task_dictionary, final_bias, 0, 
