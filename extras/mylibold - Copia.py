...
def order_queue_edd(task_q, elapsed_time, Task_dictionary, capacity, n_agents):
    ...
    i = 0
    while i < len(total_task_q_ord):
        for j in range(n_agents):
            task_q_ord.append(total_task_q_ord[i+j])
...
def Do(adjacency_matrix, non_split_tasks, tasks_dictionary, elapsed_time,
    ...
    for agent in range(n_agents):
        ...
        if agent == 0:
            task_queue = non_split_tasks[0:split_point]
            last_split = split_point
        else:
            task_queue = non_split_tasks[last_split:split_point]
            last_split = split_point
...
'''
def add_random_task(task_queue, Task_dictionary, task_scores, bias, trigger, A = 9, B = 6.5):
    n_a = list({k: v for k, v in Task_dictionary.items() if v[-1] == 'A'}.keys())
    n_b = list({k: v for k, v in Task_dictionary.items() if v[-1] == 'B'}.keys())
    n_c = list({k: v for k, v in Task_dictionary.items() if v[-1] == 'C'}.keys())
    aux = round(rd.random() * 10, 2)
...
def create_input(total_task_queue, task_dict, bias, current_position, elapsed_time, 
                 adjacency_matrix, warehouse_number, agent_capacity, n_agents):   
    input_v = []
    ...
    for agent in range(n_agents):
        ...
        for i in range(len(task_queue)):
            ...
            input_v.append(bias[i+len(task_queue)*agent])
            
    input_v.append(current_position)
    return input_v

def Split_queue(Task_queue, agent_capacity, n_agents):
    aux = Task_queue.copy()
    task_queue = []
...
def Test(adjacency_matrix, total_task_queue, tasks_dictionary, elapsed_time, idle, 
         current_position, previous_position, warehouse_number, complete, failed, changed, trigger,
         time_start_task, agent_capacity, n_agents):
    i = 0
    curr_pos = current_position
                    ...
                    if not agent_is_full: #If the agent is not full Load
                        for j in  range(len(cargo[agent])):
                            if cargo[agent][j] == 0:
                                cargo[agent][j] = curr_task
                                break
                        complete.append(str(task_queue[i]) + '.L')
                    ...
                    if curr_task not in cargo[agent]:
                        ...
                        for j in range(len(cargo[agent])):
                            if cargo[agent][j] == curr_task:
                                ...
                                agent_is_full = False
                                break
                        complete.append(str(task_queue[i]) + '.U')
    ...
    return total_task_queue[1:], idle, elapsed_time, curr_pos, prev_pos, complete,\
           failed, changed, trigger, time_start_task
...
def Train_model(task_q, task_scores, adjacency_matrix, trainning_duration, Task_dictionary, 
                warehouse_number, gamma, trigger, agent_capacity, n_agents, losses):
    elapsed_time = 0
    idling = 0
    current_position = 0
    previous_position = 0
    ...
    input_vector = create_input(task_q, Task_dictionary, bias, current_position, 
                                ...
                                agent_capacity, n_agents)
    last_reward = 0
    scores = []
    brain = Dqn(task_q, Task_dictionary, adjacency_matrix, warehouse_number, 
        gamma, input_vector, bias, current_position, losses)
    if check_brain():
        brain.load()
    ...
    while t_f - t_i <= t_d:
        ...
        task_q, idling, elapsed_time, current_position, previous_position, complete,\
        failed, changed, test_trigger, time_start_task =\
        Test(adjacency_matrix, task_q, Task_dictionary, elapsed_time, idling, 
             current_position, previous_position, warehouse_number, complete,
             failed, changed, test_trigger, time_start_task, agent_capacity, n_agents)
        bias = bias[0][1:]
        bias = bias.unsqueeze(0)
        
        if i % 2 == 0:
            task_q.remove('Dummy.D')
            task_q, bias, task_scores, test_trigger = add_random_task(task_q, Task_dictionary,
                                                        task_scores, bias, test_trigger)
        else:
            task_q.append('Dummy.D')
            bias = bias.tolist()[0]
            bias.append(0)
...
def create_bias(task_queue, Task_dictionary, current_position, elapsed_time,
                ...
                scores, agent_capacity, n_agents, losses):
