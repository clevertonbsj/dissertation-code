...
def Train_model(task_q, task_scores, adjacency_matrix, trainning_duration, Task_dictionary, 
                warehouse_number, gamma, trigger, agent_capacity, n_agents, master_loss, agent_loss, previous_duration, selector = False):
    scores = []
    total_penalty = 0
    elapsed_time = 0
    ...
    if check_brain('master ' + str(previous_duration)):
        master.load('master ' + str(previous_duration))
    if check_brain('agent ' + str(previous_duration)):
        agent.load('agent ' + str(previous_duration))
    ...
    master.save('master ' + str(t_d))
    agent.save('agent '+ str(t_d))
...
def create_queue2(task_queue, Task_dictionary, current_position, elapsed_time,
                ...
                scores, agent_capacity, n_agents, master_loss, agent_loss, training_duration, selector = False):
    ...
    master.load('master ' + str(training_duration))
    if not selector:
            master_last_state = master_input_vector
            master_bias = master.update(last_reward, master_last_state)
            master_bias = master_bias.tolist()[0]
    ...
    agent.load('agent ' + str(training_duration))
