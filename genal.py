...
for z in range(40):
    ...
    ml.create_excel(Tasks, 'tasks_40.xlsx', str(z+1))
    ml.create_excel(matrix, 'matrix_40.xlsx', str(z+1))
    task_q2 = task_q
    ...
    complete_percent_edd = len(complete_tasks2) / len(task_q)
    t_task_edd_mean = sum(t_task2)/len(t_task2)
    idle_edd_mean = sum(idle_time2)/len(idle_time2)
    if z == 1:
    ...
    complete_percent_ga = len(complete_tasks) / len(task_q)
    t_task_ga_mean = sum(t_task)/len(t_task)
    idle_ga_mean = sum(idle_time)/len(idle_time)
    if z == 1:
    ...
    while True:
        if train == True:
            ...
            for i in range(len(durations)):
                ...
                if z == 1:
                    ml.plot(complete_tasks3, t_task3, idle_time3, 'RNA treinamento = ' + str(round(3600*sum(durations[0:i+1]))) + '(s)' + ' = ' + str(round(sum(durations[0:i+1]))) + '(h)')
        
        complete_percent_rna = len(complete_tasks3) / len(task_q)
        t_task_rna_mean = sum(t_task3)/len(t_task3)
        idle_rna_mean = sum(idle_time3)/len(idle_time3)
    ...
    mean_tt_rna.append(t_task_rna_mean)
    mean_it_rna.append(idle_rna_mean)
    percent_complete_rna.append(complete_percent_rna)

ml.newplot(mean_tt_edd, mean_tt_ga, mean_tt_rna, 'Tempo médio para completar uma tarefa', 30)
ml.newplot(mean_it_edd, mean_it_ga, mean_it_rna, 'Tempo médio em ócio', 15)
ml.newplot(percent_complete_edd, percent_complete_ga, percent_complete_rna, 'Percentual de tarefas concluídas', 1)
